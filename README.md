...
-- 使用mysql 数据库
USE mysql;
-- 为mysql创建用户：test 密码为：pass123
create user test identified by 'pheonix';
-- 查看下用户
select * from mysql.user t where t.user='test';
-- 查看下用户test的权限
SHOW GRANTS FOR test;
-- 给用户case_dev在数据库名为auto_dev上赋EXECUTE(执行存储过程),INSERT,SELECT,UPDATE权限，@'%'表示从任意ip都可以访问到这个数据库
GRANT EXECUTE,INSERT,SELECT,UPDATE ON test.* TO 'test'@'%';
-- 生效
FLUSH PRIVILEGES;
-- 再次查询 下权限
SELECT *  FROM mysql.user t WHERE t.user='test' ;
...
REVOKE INSERT ON *.* FROM 'auto_dev'@'localhost';
